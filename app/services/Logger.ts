...
module Services {
    ...
    export class Logger {
        ...
        public getLogFnInner(moduleId, fnName): any {
            ...
            return (msg, data, showToast) => {
                logFn(msg /*this.localize.getLocalizedString(msg)*/, data, moduleId, (showToast === undefined) ? true : showToast);
            };
        }
        ...
        private logIt(message, data, source, showToast, toastType): void {
            ...
            if (showToast) {
                message = message; //this.localize.getLocalizedString(message);
            }
    ...
    //Register DI    
    angular.module($injections.Constants.AppName).factory($injections.Services.Logger, ['$log', /*'localize',*/ ($log, localize) => new Logger($log/*, localize*/)]);
}